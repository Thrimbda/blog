---
"title": "RDD 论文笔记"
"summary": "本文是一篇关于 Resilient Distributed Datasets (RDDs) 的论文笔记，详细阐述了 RDD
  作为分布式系统内存抽象的动机、基本特性、优势与限制。RDD
  通过牺牲计算的细粒度，实现了高效的迭代式计算、容错性和通用性，解决了传统分布式计算框架在数据复用上的性能瓶颈。文章还介绍了 RDD 在 Spark
  中的实现，包括其编程接口、依赖关系和性能评估，展示了 RDD 在迭代任务和用户分析程序上相比 Hadoop 的显著性能提升。最后，文章总结了 RDD
  的核心价值在于对集群内存的建模，使得快速连续计算成为可能，并指出硬件限制是软件创新的理论极限。"
"tags":
  - "RDD"
  - "Spark"
  - "分布式系统"
