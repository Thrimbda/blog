---
"title": "初探并发"
"summary": "本文介绍了作者通过一个小作业接触并发编程的经历，使用Python的queue、signal和time模块实现了一个简单的并发控制dem\
  o。文章解释了三个模块的作用：queue作为消息队列，signal用于捕捉KeyboardInterrupt信号，time让主线程能够捕捉到中断。作者进一\
  步探讨了多线程编程中消息队列的重要性，以及系统设计应封装细节、使用事件协同工作以降低耦合度的思想，强调“为变化而设计”的理念。"
"tags":
  - "python"
  - "并发"
  - "多线程"
