...
"summary": "本文详细介绍了作者如何将机核（Gcores）上的机组内容迁移到个人博客的技术过程。文章从旧博客因依赖过时失效开始，讲述了新博客的技术选\
  型（使用Zola生成器和简单主题），重点描述了通过API抓取机组数据、处理成Markdown格式，并利用前端技术（HTML、CSS、JavaScript）\
  实现滑动图片组交互的完整流程。作者强调了前端开发中快速反馈循环带来的快乐，并反思了这次技术实践的意义。"
"tags":
  - "博客"
  - "技术"
  - "前端开发"
