...
"summary": "本文详细记录了作者因旧博客失效而重建新博客的过程。作者选择了Zola作为生成器，并重点介绍了如何通过分析机核API获取个人机组数据，并\
  利用RxJS处理数据流，最终生成Markdown文档。此外，文章还深入探讨了如何通过编写HTML、CSS和JavaScript实现滑动图片组的前端交互效果\
  ，并分享了编码过程中反馈循环带来的快乐。文章不仅展示了具体的技术实现，还反思了工作与快乐的关系，强调高效反馈循环对创造力的重要性。"
"tags":
  - "博客"
  - "技术"
  - "前端开发"
