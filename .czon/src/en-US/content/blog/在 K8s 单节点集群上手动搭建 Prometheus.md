...
"summary": "This article is a technical tutorial aimed at guiding readers to manually deploy the Prometheus monitoring system on a single-node Kubernetes cluster, without relying on quick tools like Helm Charts or the Prometheus Operator. It first introduces the basic operation and configuration of Prometheus through a bare-metal proof-of-concept. Then, it elaborates on the various resources required for deployment in a K8s environment, including Namespace, DaemonSet, ConfigMap, ServiceAccount, ClusterRole, and more. The core section explains how to configure Prometheus's service discovery (specifically kubernetes_sd_config) and relabeling (relabel_config) to monitor multiple targets, such as Prometheus itself, Node Exporter, Kubelet, cAdvisor, and the API Server. The article also emphasizes the importance of RBAC permission configuration and provides specific YAML configuration examples. Finally, the author shares a collection of resource declarations accumulated from practical experience to help readers complete the deployment."
"tags":
  - "Observability"
  - "Prometheus"
  - "Kubernetes"
...
> The target audience for this article is those who are just starting to learn about monitoring systems and have limited knowledge of Prometheus (much like the author was when writing this article).
>
>
>
> Environment used to set up Prometheus in this article:
>
> - K8s version: 1.19.3
> - Prometheus version: 2.22.0
> - Operating System: Archlinux as of 2020.11
> - Hosts file configured, domain name for Devbox is `devbox`
>
> ⚠️ Please note: The command-line parameters listed in this article need to be adjusted slightly according to your current environment (e.g., Prometheus binary version, etc.).
>
>
>
...
> 3. [Introduction on the official Prometheus website](https://prometheus.io/docs/introduction/overview/)
...
2. Setting up Prometheus on K8s means K8s is responsible for managing the Prometheus service. Unlike the Prometheus Operator mentioned above, here we need to write the various related YAML configuration files ourselves.
...
## Bare-Metal Prometheus Proof of Concept
...
> I tried to be lazy and searched for tutorial blogs, but found them unclear and mostly outdated. Wasted half a day and ended up having to honestly read the official documentation.
...
You can see Prometheus has started. Access http://devbox:9090 to see its web UI. Click around randomly to get a general feel for the features Prometheus provides, giving us an understanding of how Prometheus behaves when running normally.
...
> Note: As a proof-of-concept, it is not recommended to directly use a bare-metal deployed Prometheus to monitor a K8s cluster. The reason is that accessing K8s components from outside the cluster requires certificate configuration and a [ClusterRole](https://kubernetes.io/docs/reference/access-authn-authz/rbac/) with appropriate access permissions (omitting the various pitfalls the author encountered when trying to use a bare-metal Prometheus to monitor a K8s cluster and its components).
...
Here's a brief description of the following configuration items (they are not necessarily orthogonal):
...
It seems the core configuration item in Prometheus is its `<scrape_config>`. Each one defines a monitoring job, similar to a namespace concept, mainly providing an aggregation of monitoring targets. Within it, we tell Prometheus specifically from which endpoints to pull data and how to filter these endpoints by defining `<*_sd_config>` or `<static_config>`.
...
On a K8s cluster with RBAC applied, we need to define a role with sufficient permissions for Prometheus to read the cluster state and various metrics, hence items 5-7.
...
Combining the introduction to Prometheus configuration from earlier, define four monitoring targets with four `<scrape_config>`s:
