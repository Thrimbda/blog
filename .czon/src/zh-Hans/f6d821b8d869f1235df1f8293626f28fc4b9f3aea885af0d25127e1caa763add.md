---
"title": "二探并发(Actor)"
"summary": "文章首先讨论了在并发编程中，当多个worker需要异步工作且各司其职时，共享一个消息队列会导致消息无法定向发送、错收消息以及时序问题。\
  作者通过逐步分析，指出共享消息队列在这种场景下的效率低下和潜在bug。接着，文章引入了Actor模型作为根治方案，该模型为每个worker分配独立的消息队\
  列（邮箱），实现完全的消息隔离和异步通信。文章详细解释了Actor模型的概念，包括其并行执行、消息传递和地址机制，并提供了一个Python线程实现的示例代\
  码，展示了如何构建一个基本的Actor（BaseWorker）。最后，作者总结了Actor模型在并发编程中的优势，强调其抽象思维和“高内聚，低耦合”的实践\
  价值。"
"tags":
  - "python"
  - "并发"
