---
"title": "Zola 博客 WYSIWYG 编辑与 GitHub 后端整合方案设计"
"summary": "本文档详细规划了为现有 Zola 博客添加 WYSIWYG 实时 Markdown
  编辑功能的方案，核心目标是保持现有设计风格，避免引入手写后端。通过分析现状和设计原则，文档推荐了以纯前端实现、使用 GitHub OAuth device
  flow 进行鉴权、将草稿保存在本机、并通过创建 PR 将内容发布回 GitHub
  仓库的架构。该方案确保了编辑体验的实时预览和自动保存，同时明确了内容同步路径，不破坏现有结构。文档还讨论了其他候选方案、安全风险以及实施阶段。"
"tags":
  - "Zola"
  - "WYSIWYG"
  - "Markdown"
