---
"title": "观察者模式实力分析"
"summary": "本文首先介绍了观察者模式（又称发布-订阅模式）的基本概念、GoF定义及其在真实世界中的应用（如RSS订阅）。文章详细解释了观察者模式的\
  参与者（Subject、Observer、ConcreteSubject、ConcreteObserver）和适用场景（对象间存在依赖关系、一个对象更改需\
  同时更改其他对象）。随后，文章以J2EE中的Action和ActionListener为例，通过一个具体的Java
  Swing代码实例，展示了观察者模式如何应用于MVC范式以降低model和view的耦合。文章指出，相比于普通函数调用和回调函数，观察者模式能够低成本维持\
  对象间一致性，实现广播效果，并显著提高代码可重用性。"
"tags":
  - "设计模式"
  - "系统分析与设计"
  - "观察者模式"
