---
"title": "二探并发(Actor)"
"summary": "本文深入探讨了并发编程中共享消息队列的局限性，指出在需要各worker异步工作的系统中，共享队列会导致消息无法定向发送、时序错乱和效率\
  低下等问题。作者通过分析逐步引入标识、回放机制等解决方案的缺陷，最终提出Actor模型作为根治方案。文章详细解释了Actor模型的概念，包括其并行执行、消\
  息解耦和地址通信等核心特性，并提供了一个Python线程实现的示例代码。总结强调并发编程需要抽象思维，Actor模型是入门并发领域的有效指引。"
"tags":
  - "python"
  - "并发"
  - "Actor模型"
