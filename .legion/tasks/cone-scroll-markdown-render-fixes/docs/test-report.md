...
- `grep -n '^#{1,6} ' content/blog/markdown-render-showcase.md`
...
- 已验证事实：`node scripts/zola-i18n.ts build` 成功完成，输出 `generated=240`，Zola 构建完成并创建 285 pages / 14 sections。
- 已验证事实：`content/blog/markdown-render-showcase.md` 现已显式包含 `#` 到 `######` 的 1-6 级标题样本，可继续用于观察标题层级、TOC 与锚点表现。
...
- 已验证事实：无序列表与有序列表现在共享相同的 `padding-left` 与 marker 宽度；Playwright 断言确认首页两类列表的 marker 列宽一致，不再出现明显的两套缩进节奏。
...
- 本轮还收口了一个视觉细节：之前有序列表为数字 marker 预留了固定宽度，但无序列表没有共享同一条 marker 列，所以读者会感觉两类列表缩进不一致；现已统一 marker 列宽与文本起始列。
