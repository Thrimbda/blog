...
"summary": "本文详细介绍了作者为理解RBAC模型而开发的一个名为“为了艾尔”的Demo项目。该项目模拟了星际争霸游戏中的简化场景，使用Python\
  的Flask框架构建了一个RESTful服务，并在此服务中实现了RBAC模型。文章阐述了Demo的设计思路，包括主体、角色、资源和权限的定义，以及主体-角\
  色和角色-权限的分配关系。同时，文章解释了如何在不使用数据库的情况下，通过文件配置来管理RBAC关系，并将RBAC与RESTful架构相结合。最后，文章介\
  绍了Demo的业务逻辑和核心代码实现，展示了如何通过角色控制对游戏资源的操作，从而完成游戏目标。"
"tags":
  - "RBAC"
  - "RESTful"
  - "Flask"
