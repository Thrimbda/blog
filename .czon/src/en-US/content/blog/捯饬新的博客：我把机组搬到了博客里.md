---
"title": "Tinkering with a New Blog: I've Moved My Gcores Talks into the Blog"
"summary": "This article details the technical process of migrating the author's Gcores Talks content to a personal blog. It begins with the old blog breaking down due to outdated dependencies, explains the choice of Zola as the new static site generator, and focuses on the process of fetching talk data via API, converting it to Markdown, and implementing a swipeable image gallery frontend using HTML, CSS, and JavaScript. The author emphasizes the joy derived from the rapid feedback loop in frontend development and shares the RxJS implementation for touch interactions. Finally, the article concludes with the satisfaction of having complete control over the blog and the pure pleasure of coding, acknowledging the content might be shallow but the process was immensely rewarding."
"tags":
  - "Blog"
  - "Technology"
...
In April, during a trip, I had a strong urge to write a travelogue. I first posted it on Gcores. Following my usual routine, I would then organize a Markdown version to publish on my personal blog.

However, after editing it, I found my blog was broken.

## The Old Blog

The trend of building personal blogs became popular in our department during my junior year. Seven years ago, [this](./搭好博客后的一点话.md) cringe-worthy article described my initial thoughts and feelings when I first set up this blog.
...
After writing, I would nonchalantly execute a few commands to push the new blog update:
...
Then, eagerly opening the CI interface—CI was still somewhat trendy back then, the DevOps concept had only been proposed a few years prior. On the CI log page, I'd wait for the entire blog, including this new article, to be built into a static HTML site, wait for Travis to push the build artifacts to the git repository using my key, and wait for Github to publish the build artifacts.
...
Perhaps because it was new enough, or because the technology choices were simple enough, or maybe because the feedback loop in frontend work is short enough, coupled with inevitably picking up some frontend knowledge while working with Azheng, filling the knowledge gap I neglected in my freshman year, the process of tinkering with the blog became fun again.
...
I want to ride the tailwind of this joy to elaborate (self-indulgently) on the technical process of moving my Gcores Talks into the blog.
...
During a previous exploration, I discovered that Gcores is incredibly friendly to crawlers. Its data APIs are very uniform and expressive. For example, exploring my talks:

![gcores-api-at-glance](https://0xc1.space/images/2024/06/18/gcores-api-at-glance.jpg)

Found that this is the API used to request talk data:
...
Now we have an Rx stream containing all the raw talk data for a user.
...
Talks are similar to social media posts like Weibo or Twitter—text with images, plus some tags. Let's display them in a flat layout.
...
Combined with our desired output, we define the following structure:
...
Now we have a collection of our desired data structures. Next, we just need to process them into `string` format.
...
The complete version of this code is [here](https://github.com/Thrimbda/blog/blob/184bad3cbc217b52248dfe169a09fb44aa551328/scripts/get-gcores-talk.ts). Run it with `deno`:
...
At this point, we have output a valid Markdown file, sufficient to render into a decent-looking webpage.

![gcores-pictures-render](https://0xc1.space/images/2024/06/18/gcores-pictures-render.jpg)

But the images look odd: directly rendering them as flat Markdown stacks makes the proportion between images and text very unbalanced, giving a top-heavy feeling. It would be great if we could make them into a swipeable image gallery like on Gcores.
...
Wait? Writing HTML + CSS + JS? This takes me back to freshman year.
...
-   A container for the swipeable gallery itself.
-   The images.
...
Thanks to more modern frontend technologies, implementing this with flexbox is now very simple. The core idea is to arrange the images in a single row but only reveal the space for one image, hiding the others behind it.
...
-   Mouse clicks on the left/right buttons.
-   Mouse clicks on the small dots.
-   Left/right swiping on touch devices.

With web APIs having subsumed jQuery's functionality, and with the help of RxJS, this becomes quite straightforward:
...
The gist of the entire code is that for each swipeable gallery, we record a current display index: `[0, 1, ..., imageNumbers - 1]`. User interaction changes this index, which then calculates how many pixels the gallery should offset to display the image indicated by the index.

I'm quite proud of the final Rx code for touch interaction.
...
Work can bring joy because our brains love feedback loops.
...
stateDiagram-v2
    ...
    ViewResult --> Satisfied
    Satisfied --> [*]

{% end %}

Pain comes from incomplete or overly long cycles. Writing frontend code is so joyful because its cycle is highly efficient. Each time you modify the code, with a save and refresh, you can immediately see the effect of the change. This allows for several such feedback loops within minutes, thereby bringing joy.

I hope this kind of joy can accompany everyone.

## Conclusion

To this day, the content discussed in this article could still be considered somewhat progressive for a university student's blog, but its depth is rather shallow relative to my years of work experience, to the point where I often felt embarrassed while writing it.
