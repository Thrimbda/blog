...
- 编辑任意 `content/**.md` 文件
- 草稿自动保存到本机（IndexedDB）
- 发布时将 Markdown 和图片写回 GitHub 仓库（默认：创建分支 + PR）
...
2.  创建 GitHub OAuth App 并填入 `oauthClientId`
    ...
    -   Callback URL: `https://0xc1.space/admin/`（设备流不需要，但 GitHub 表单需要填写）

> `config.js` 不应包含任何密钥；这里只需要 `client_id`。
>
> 如果你希望**发布后的站点也能直接使用 `/admin/`**，请将 `static/admin/config.js` 一起提交到仓库（它不包含密钥，公开也没问题）。

## 使用

1.  打开 `/admin/`
2.  点击 “Connect (Device Flow)”
3.  按照页面提示前往 GitHub 输入代码完成授权
4.  刷新文件列表，加载 `content/**.md` 文件
5.  编辑内容（自动保存草稿到本机）
6.  插入图片（会在发布时写入 `static/images/uploads/...`）
