---
"title": "源码阅读：Harbor-Operator 设计分析"
"summary": "本文详细解析了 Harbor-Operator 的源码设计，重点探讨了其如何通过依赖图（Dependency
  Graph）解决资源间依赖关系，使用单一控制器（Controller）实现多个 CRD 的调谐逻辑，以及通过 ResourceManager
  将代码配置化以提高复用率。文章还分析了 Harbor-Operator 在 Golang 和 Kubernetes 平台上的实践，展示了其作为 'the
  right thing' 设计理念的典范，如何在简单性、一致性和完整性之间取得平衡，为 Operator 开发提供了宝贵的设计参考。"
"tags":
  - "Harbor-Operator"
  - "源码分析"
  - "Kubernetes"
