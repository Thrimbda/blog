---
"title": "新しいブログをいじくり回す：機核の「機組」をブログに移した話"
"summary": "本記事では、著者が旧ブログの機能停止をきっかけに新ブログを再構築した過程を詳細に記録しています。著者はZolaをジェネレーターとして選択し、機核APIを分析して個人の「機組」データを取得し、RxJSを用いてデータストリームを処理し、最終的にMarkdownドキュメントを生成する方法に焦点を当てています。さらに、HTML、CSS、JavaScriptを記述してスライド式画像ギャラリーのフロントエンドインタラクションを実装する方法について深く掘り下げ、コーディングプロセスにおけるフィードバックループの楽しさについても共有しています。本記事は具体的な技術実装を示すだけでなく、仕事と楽しさの関係についても考察し、創造性に対する効率的なフィードバックループの重要性を強調しています。"
"tags":
  - "ブログ"
  - "技術"
  - "フロントエンド開発"
...
4月、ある旅行中に、私は強い衝動に駆られて旅行記を書き、まず機核に投稿しました。そしていつもの習慣に従い、Markdown形式に整理して自分のブログに投稿しようとしました。

ところが編集を終えた後、ブログが壊れていることに気付いたのです。

## 以前のブログ

個人ブログを構築するブームは、大学3年生の時に私たちの学科で流行りました。7年前に書いた[この記事](<[搭好博客后的一点话](./da-hao-bo-ke-hou-de-yi-dian-hua/)>)は、最初にこのブログを構築した時の私の心境を紹介しており、今読むと恥ずかしさで足の指が丸まります。

その理由は、まず自分に何かを書くことを促したかったからです。次に、学校では教えてくれない、最新で流行りの派手な技術を組み合わせたエンジニアリングの実践をしたかったからです。そして、独自ドメインを持つことは本当にクールだと思いました。少なくとも私はそう思っていました。

当時の情景を再現してみましょう。

---

少しでも新しいことを学ぶと、興奮した気持ちで、心に浮かんだアイデアをMarkdown形式のドキュメントに記録しました。

書き終えた後、粋に数行のコマンドを実行して新しいブログ記事をプッシュします：
...
そして興奮しながらCIのインターフェースを開きます。当時CIはまだ比較的新しいものでした。DevOpsの概念が提唱されてから数年しか経っていませんでした。CIのログページで、この記事を含むブログ全体が静的なhtmlウェブサイトとして構築されるのを待ち、Travisが私の鍵を使ってビルド成果物をgitリポジトリにプッシュするのを待ち、Githubがビルド成果物を公開するのを待ちました。

最後に、私のドメイン [https://blog.thrimbda.com](https://blog.thrimbda.com) にアクセスし、満足げにその新しい記事がページの一番上に表示されているのを見ました。

---

長い年月を経て、このプロセスは退屈なものになりました。退屈でありながらも効果は持続し、それがついに機能しなくなる日が来るまで続きました。

## 新しいブログ

4月末のあの日、私はCIパイプラインを3回連続で再試行しましたが、すべて失敗しました。ログをざっと見ると、ビルドに使用されるある依存関係が古すぎてインターネット上から消えてしまったようでした。私は、この業界において7年という時間がどれほど巨大なスパンであるかを認識しました。砂嵐が城を破壊し、海水が山頂を飲み込むほどに長い時間です。

そこで、仕事の合間の「サボり時間」に、あまり面白くはない要件を携えて技術選定を行いました：**十分にシンプルで、十分に便利であること。**

この新しいブログはそうして生まれました。いつもの習慣に従い、紹介します：
...
おそらく十分に新しいからか、技術選定が十分にシンプルだからか、あるいはフロントエンド作業のフィードバックループが十分に短いからか、阿錚と一緒に仕事をしているうちに自然とフロントエンドの知識が身につき、大学1年生の時にあまり真剣に学ばなかった知識の穴を埋めることができたからかもしれません。ブログをいじくり回す過程は再び面白いものになりました。

ちょっとしたCSSスタイルの修正、新しいgitcusの追加、コメント欄をgithubにホストするなど。私は7年前の自分と意見が一致しました：コーディングとエンジニアリングをいじくり回すことは、やはり楽しいことです。それは労働の喜びであり、遊びの喜びです。

私はこの喜びの余韻に乗じて、自分の「機組」をブログに移した技術的な過程を（自己満足で）詳しく話したいと思います。
...
以前のある探索で、私は機核がクローラーに対して非常に親切であることに気付いていました。そのデータインターフェースは非常に統一されており、表現力に富んでいます。例えば、私の「機組」を探索してみましょう：
...
さらに観察すると、ページネーションは `before` というパラメータで決定されているようです。このパラメータのタイムスタンプを前に進め続け、データが取得できなくなるまで繰り返せばよいようです。これにより、以下のようなコードが得られます：
...
生データが得られたので、次はMarkdownに処理します。直接処理を始めましょう。重要な点は以下の通りです：

- 表示する内容を決定する
- 機核のインターフェースフィールドを理解する

「機組」は、SNSのタイムラインや微博と似ており、テキストと画像、それにいくつかのタグで構成されています。そのまま平らに表示しましょう。
...
出力したい内容と組み合わせて、以下のような構造体を定義します：
...
出力するのは、`string` 型のMarkdownテキストで十分です。
...
これで、欲しかったデータ構造が大量に得られました。次は `string` に処理するだけです。
...
私は最も強引な処理方法を選択し、直接近くのディレクトリに出力しました。
...
これで、有効なMarkdownが出力され、十分に見栄えのするウェブページとしてレンダリングできるようになりました。

![gcores-pictures-render](https://0xc1.space/images/2024/06/18/gcores-pictures-render.jpg)

しかし、画像は奇妙に見えます：Markdownを直接レンダリングして平らに積み重ねると、画像とテキストの比率が非常に不調和になり、頭でっかちな印象を与えます。機核のようにスライド式の画像ギャラリーにできればいいのにと思います。

#### ソリューションの選定

MarkdownはHTMLを通じてその表現力を拡張することをサポートしているので、どのみち独自のコードで画像を直接HTMLとしてレンダリングし、望む効果を得ることができます。しかし、Zolaは別の方法を提供しており、目標をより簡単に実現できます：[Shortcodes | Zola](https://www.getzola.org/documentation/content/shortcodes/)

私たちがやるべきことは、適切なCSSでスタイルを提供し、JSコードでインタラクションを担当するHTMLテンプレートを記述することです。そして、本文のMarkdownで直接このHTMLを呼び出します。

待って？HTML + CSS + JSを書くってこと？大学1年生に戻った気分です。

#### フロントエンドの作成

##### HTML

HTMLの部分は非常にシンプルで退屈です。各画像ギャラリーに対して以下の要素を提供します：
...
よりモダンなフロントエンド技術のおかげで、flexレイアウトを使ってこれを実装するのは今では非常に簡単です。核心となる考え方は、画像を一列に並べるが、1枚の画像の位置だけを表示し、他の画像はすべて後ろに隠すことです：
...
タッチインタラクションはボタンインタラクションに比べてより複雑です。なぜなら、ユーザーがスワイプしている間、画像はユーザーの指と一緒に動かなければならず、この感触を適切に調整しないとユーザーをがっかりさせてしまうからです。

この複雑さは、インタラクションにより多くの情報と、より複雑なWEB APIを必要とすることを意味します。

インタラクションロジックの観点から、タッチ開始時の指のピクセル位置と、初期位置に対する各レンダリングフレームでの指のピクセル位置のオフセットを記録する必要があります。そして、各フレームのレンダリング時にこのオフセットをスライド画像ギャラリーに適用し、画像が指と一緒に動くようにします。ある距離を移動した後、指が画面から離れたときに、画像を切り替えるかどうかを決定します。例えば、指が画面を軽くタップしただけで、2ピクセルの右オフセットが生じた場合、画像は切り替わるべきではありません。一方、指が画面の半分を右にスワイプした場合、画像は切り替わるべきです。オフセットとコンテナ全体の幅に対するしきい値を20%に設定してみましょう。指のスワイプ距離が画像ギャラリー幅の20%を超えた場合、画像を切り替えるべきだと判断します。
...
仕事は人に楽しさをもたらします。なぜなら、私たちの脳はフィードバックとループを好むからです。
...
苦痛は、不完全なループや長すぎるループから生まれます。フロントエンドコードを書くことはとても楽しいです。この楽しさは、そのループが十分に効率的だからです。コードを変更するたびに、保存と更新を伴い、変更の効果を直接見ることができます。そのため、数分で何度もこのフィードバックループを完了でき、楽しさをもたらします。

この楽しさが皆さんに常に付き添うことを願っています。

## まとめ

今日に至るまで、この記事が述べている内容は、大学生のブログとしてはまだ意欲的ですが、私の職歴に比べるとその深さはかなり浅く、書いている間、しばしば恥ずかしさを感じました。

しかし、いずれにせよ、前回ブログを構築した時の丸写しに比べ、今回は自分のブログのあらゆる部分を完全にコントロールできました。いじくり回す過程は、非常に純粋でシンプルな楽しさをもたらし、まさにこの楽しさが、私に厚かましくもこれを書き上げさせたのです。