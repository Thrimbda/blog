...
"summary": "本文档是一份关于为现有 Zola 静态博客站点添加 WYSIWYG
  实时编辑功能的设计方案。文档首先进行了现状审计，确认了站点技术栈（Zola + GitHub Actions 部署）和必须保持的设计语言（基于
  suCSS.css 的 CSS 变量和主题切换）。核心部分调研了多个 Markdown 编辑器（重点推荐 Vditor）和免手写后端方案（重点推荐
  Supabase），并对比了各自的优缺点和适配性。文档还明确了待决策的关键问题，如“实时编辑”的具体含义、编辑范围以及发布链路。最终目标是设计一个无需引入\
  复杂构建链、能保持现有站点风格、并能安全处理 Zola 短代码和 Front Matter 的编辑发布系统。"
"tags":
  - "Zola"
  - "WYSIWYG"
  - "Markdown 编辑器"
