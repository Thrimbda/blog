...
"summary": "本記事では、著者が旧ブログの技術的陳腐化を理由に新ブログを再構築し、特に技術的手段を用いて機核（Gcores）上の個人「機組」コンテンツを収集、処理し、新ブログに統合した全過程を詳細に記録しています。技術選定（Zola静的サイトジェネレーター）から始まり、機核APIからのデータ取得、RxJSを用いたページネーションとデータ構造変換、Markdownファイルの生成、そしてZolaのShortcodes機能を利用したスライド式画像ギャラリーのフロントエンド実装に至るまで、各工程を段階的に解説しています。著者は具体的なコード実装を共有するだけでなく、フロントエンド開発がもたらす迅速なフィードバックループの楽しさについても考察し、最終的には技術的制御力と創造過程の純粋な喜びに対する満足感を表明しています。"
"tags":
  - "ブログ"
  - "技術"
  - "フロントエンド開発"
...
4月、ある旅の途中で、私は強い衝動に駆られて一篇の旅行記を書き、まず機核に投稿しました。そして慣例に従い、Markdown形式に整理して自分のブログにも投稿しようとしました。

ところが編集を終えた後、ブログが壊れていることに気づいたのです。

## 以前のブログ

個人ブログを構築するブームは、大学3年生の時に私たちの学科で流行りました。7年前に書いた[この記事](./搭好博客后的一点话.md)（今読むと恥ずかしさで足の指が丸まるような文章です）は、私が初めてこのブログを構築した時の心境を紹介しています。

その理由は、まず自分に何かを書くことを促したかったからです。次に、学校では教えてくれない、最新で流行りの派手な技術を組み合わせたエンジニアリングの実践をしたかったからです。そして、独自ドメインを持つことは本当にクールだと思いました、少なくとも私にはそう思えました。

当時の情景を再現してみましょう。

---

少しでも新しいことを学ぶと、興奮した気持ちで、心に浮かんだ考えをすぐにMarkdown形式のドキュメントに記録しました。

書き終えた後、さっそうと数行のコマンドを実行して新しいブログをプッシュします：
...
それから興奮してCIのインターフェースを開きます。当時CIはまだ比較的新しいものでした。DevOpsの概念が提唱されてから数年しか経っていません。CIのログページで、この記事を含むブログ全体が静的HTMLサイトとして構築されるのを待ち、Travisが私の鍵を使ってビルド成果物をGitリポジトリにプッシュするのを待ち、Githubがビルド成果物を公開するのを待ちます。

最後に私のドメイン [https://blog.thrimbda.com](https://blog.thrimbda.com) にアクセスし、その新しい記事がページの一番上に表示されているのを満足げに見るのでした。

---

長い年月を経て、このプロセスは退屈なものになり、退屈でありながらも効果的であり続け、ついにそれが機能しなくなる日が訪れました。

## 新しいブログ

4月末のある日、私はCIパイプラインを3回連続で再実行しましたが、すべて失敗しました。ログをざっと見ると、ビルドに使用されていたある依存関係が、古すぎるためにインターネット上から消えてしまったようでした。私は、7年という時間がこの業界においてどれほど巨大なスパンであるかを認識しました。砂嵐が城を壊し、海水が山を飲み込むほどに長い時間です。

そこで、仕事の合間の「サボり時間」に、あまり面白くはない要件をいくつか持って技術選定を行いました：**十分にシンプルで、十分に便利なもの。**

この新しいブログはそうして生まれました。慣例に従い、紹介します：
...
おそらく十分に新しいからか、技術選定が十分にシンプルだったからか、あるいはフロントエンド作業のフィードバックループが十分に短いからか、そして阿錚と一緒に仕事をしていると自然とフロントエンドの知識が耳に入り、大学1年生の時にあまり真剣に学ばなかった知識の穴を埋めることができたからか、ブログをいじくり回す過程が再び面白くなりました。

小手調べのCSSスタイル修正、新しいgitcus（Githubにホストされたコメント機能）の追加。私は7年前の自分と意見が一致しました：コーディングとエンジニアリングのいじくり回しは、やはり楽しいことです。労働の喜びであり、遊びの喜びです。

私はこの喜びの余韻に乗じて、自分の「機組」をブログに移した技術的過程を（独りよがりで）詳しく説明したいと思います。
...
以前のある探索で、私は機核がクローラーに対して非常に友好的であることを発見していました。そのデータインターフェースは非常に統一されており、表現力に富んでいます。例えば、私の「機組」を探索してみましょう：

![gcores-api-at-glance](https://0xc1.space/images/2024/06/18/gcores-api-at-glance.jpg)

このAPIが「機組」データをリクエストするためのものであることがわかります。
...
さらに観察すると、ページネーションは `before` というパラメーターで決定されているようです。このパラメーターのタイムスタンプを前方向に繰り返し更新し、データが取得できなくなるまで続ければよいのです。これにより、次のようなコードが得られます：
...
生データが得られたので、次はMarkdownに処理します。直接処理を始めましょう。ポイントは以下の通りです：

- 表示する内容を決定する
- 機核のインターフェースフィールドを理解する

「機組」は、SNSのタイムラインや微博と似ていて、文字と画像、それにいくつかのタグで構成されています。そのまま平らに表示しましょう。
...
出力するのは `string` 型のMarkdownテキストです。
...
これで、必要なデータ構造が大量に得られました。次に、これを `string` に処理するだけです。
...
このコードの完全版は[こちら](https://github.com/Thrimbda/blog/blob/184bad3cbc217b52248dfe169a09fb44aa551328/scripts/get-gcores-talk.ts)にあります。`deno` で実行します：
...
これで、有効なMarkdownが出力され、まだ見られる程度のWebページとしてレンダリングできるようになりました。

![gcores-pictures-render](https://0xc1.space/images/2024/06/18/gcores-pictures-render.jpg)

しかし、画像の見た目は奇妙です：Markdownを直接レンダリングして平らに積み重ねると、画像と文字の比率が非常に不釣り合いで、頭でっかちな印象を与えます。機核のようにスライド式の画像ギャラリーにできればいいのにと思います。
...
私たちがやるべきことは、適切なCSSでスタイルを提供し、JSコードでインタラクションを担当するHTMLテンプレートを書くことです。そして、本文のMarkdownで直接このHTMLを呼び出します。

待って？HTML + CSS + JSを書くって、大学1年生に戻ったみたいです。
...
Web APIがJQueryを取り込んだ後、rxjsの助けを借りて、これは非常に簡単です：
...
タッチインタラクションはボタンインタラクションよりも複雑です。なぜなら、ユーザーがスワイプしている間、画像はユーザーの指と一緒に動かなければならず、この感触を適切に調整しないとユーザーをがっかりさせてしまうからです。

この複雑さは、インタラクションにより多くの情報と、より複雑なWEB APIが必要であることを意味します。

インタラクションロジックの観点からは、タッチ開始時の指のピクセル位置、そして各レンダリングフレームが到来した時の指のピクセル位置の初期位置からのオフセットを記録する必要があります。そして、各フレームをレンダリングする際に、このオフセットをスライド画像ギャラリーに追加して、画像が指と一緒に動くようにします。ある距離を移動した後、指が画面から離れたときに、画像を切り替えるかどうかを決定します。例えば、指が画面を軽くタップしただけで、2ピクセルの右オフセットが生じた場合、画像は切り替わるべきではありません。一方、指が画面の半分を右にスワイプした場合、画像は切り替わるべきです。オフセットとコンテナ全体の幅の比のしきい値を20%に設定してみましょう。指のスワイプ距離が画像ギャラリー幅の20%を超えた場合、画像を切り替えるべきだと判断します。

APIの観点からは、関連するイベントは3つあります：
