---
"title": "Spark 论文笔记"
"summary": "本文是对 Spark 论文的笔记，介绍了 Spark 如何通过 RDD（弹性分布式数据集）这一数据结构，克服 MapReduce
  等系统在通用性上的不足，支持跨多个并行操作的数据重用。文章详细阐述了 RDD 的创建方式、持久化策略、编程模型（如并行操作），以及 Spark 在
  Scala 上的实现细节，包括其惰性求值和短暂性特性。实验结果显示，Spark 在特定场景下比 Hadoop 快 10 倍。最后，文章总结了 Spark
  的早期优势、局限性以及相关技术对比。"
"tags":
  - "Spark"
  - "RDD"
