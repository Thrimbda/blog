---
"title": "Am neuen Blog herumbasteln: Ich habe meine Gcores-Talks in den Blog integriert"
"summary": "Dieser Artikel dokumentiert detailliert den Neubau eines Blogs aufgrund veralteter Technik des alten und konzentriert sich auf den vollständigen Prozess, wie persönliche Talk-Inhalte von Gcores durch technische Mittel gesammelt, verarbeitet und in den neuen Blog integriert wurden. Beginnend mit der Technologieauswahl (dem Static-Site-Generator Zola) werden Schritt für Schritt die einzelnen Schritte analysiert: das Abrufen von Daten über die Gcores-API, die Verwendung von RxJS zur Verarbeitung von Paginierung und Datenstrukturtransformation, die Erzeugung von Markdown-Dateien bis hin zur Implementierung von Frontend-Interaktionen für schiebbare Bildergalerien mithilfe der Shortcodes-Funktion von Zola. Der Autor teilt nicht nur konkrete Code-Implementierungen, sondern reflektiert auch über die Freude, die der schnelle Feedback-Zyklus der Frontend-Entwicklung mit sich bringt, und drückt letztlich Zufriedenheit mit der technischen Kontrolle und der reinen Freude am Schaffensprozess aus."
"tags":
  - "Blog"
  - "Technik"
...
Nachdem ich mit dem Bearbeiten fertig war, stellte ich fest, dass der Blog kaputt war.

## Der alte Blog

Die Mode, persönliche Blogs zu erstellen, schwappte in meinem dritten Studienjahr in unserer Fakultät auf. Dieser vor 7 Jahren verfasste, [für mich heute peinlich berührende Artikel](./搭好博客后的一点话.md) beschreibt meine damaligen Gedanken und Beweggründe beim Aufbau dieses ersten Blogs.

Die Gründe waren: Erstens wollte ich mich dazu anhalten, etwas zu schreiben; zweitens wollte ich praktische Erfahrungen sammeln und ein paar coole, neue Technologien ausprobieren, die an der Uni nicht gelehrt wurden; und außerdem ist es einfach cool, eine eigene Domain zu haben – zumindest fand ich das.

Lassen Sie uns die Szene von damals nachspielen:

---

Nachdem ich etwas Neues gelernt hatte, notierte ich voller Aufregung meine Ideen in einem Markdown-Dokument.

Nach dem Schreiben führte ich lässig ein paar Befehle aus, um den neuen Blogbeitrag live zu schalten:
...
An jenem Tag Ende April scheiterten drei aufeinanderfolgende CI-Pipeline-Läufe. Ein kurzer Blick in die Logs zeigte, dass eine Build-Abhängigkeit anscheinend zu alt war und aus dem Internet verschwunden war. Da wurde mir klar, was für eine enorme Zeitspanne 7 Jahre in dieser Branche sind – lang genug, dass Sandburgen verwehen und Berge vom Meer verschluckt werden.

Also nutzte ich Leerlaufphasen bei der Arbeit, um mit einigen nicht sonderlich spannenden Anforderungen eine Technologieauswahl zu treffen: **Einfach genug, bequem genug.**
...
## Der herumbastelnde Prozess der unsterblichen Neugier

Vielleicht weil er neu genug war, oder weil die Technologieauswahl einfach genug war, vielleicht weil der Feedback-Zyklus der Frontend-Arbeit kurz genug ist, und auch weil die Arbeit mit Azheng unweigerlich etwas Frontend-Wissen vermittelt und meine Wissenslücken aus dem ersten Studienjahr schließt, wurde das Herumbasteln am Blog wieder interessant.
...
Bei einer früheren Erkundung hatte ich bereits festgestellt, dass Gcores Crawlern gegenüber äußerst freundlich ist. Seine Daten-APIs sind sehr einheitlich und ausdrucksstark. Zum Beispiel bei der Erkundung meiner Talks:

![gcores-api-at-glance](https://0xc1.space/images/2024/06/18/gcores-api-at-glance.jpg)

Man erkennt, dass diese API für die Abfrage der Talk-Daten genutzt wird:
...
Talks sind ähnlich wie Freundeskreise oder Microblogs: Text mit Bildern, plus ein paar Tags. Lassen wir sie einfach flach anzeigen.

Die Daten eines Talks können wie folgt definiert sein:
...
Basierend auf dem, was wir ausgeben wollen, definieren wir folgende Struktur:
...
const cookedData$ = rawGcoresTalkData$.pipe(
  ...
  mergeMap(({ data, included }): IGcoresTalk[] => {
    // Wir brauchen nur den Titel als Tags für den Talk
...
Ich wählte den direktesten Ansatz: Direkte Ausgabe in ein lokales Verzeichnis.
...
An diesem Punkt haben wir ein gültiges Markdown ausgegeben, das ausreicht, um eine halbwegs ansehnliche Webseite zu rendern.

![gcores-pictures-render](https://0xc1.space/images/2024/06/18/gcores-pictures-render.jpg)

Aber die Bilder sehen seltsam aus: Einfaches, flaches Stapeln durch direkte Markdown-Renderung führt zu einem unausgewogenen Verhältnis zwischen Bild und Text, einem kopflastigen Eindruck. Wäre es nicht schön, wenn sie wie bei Gcores zu einer schiebbaren Galerie werden könnten?

#### Lösungsansatz

Markdown unterstützt die Erweiterung seiner Ausdruckskraft durch HTML, also könnten wir auf jeden Fall unseren eigenen Code schreiben, um die Bilder direkt als HTML zu rendern und den gewünschten Effekt zu erzielen. Aber Zola bietet einen anderen Weg, der das Ziel einfacher erreichen lässt: [Shortcodes | Zola](https://www.getzola.org/documentation/content/shortcodes/)

Was wir tun müssen, ist, ein HTML-Template mit passendem CSS für das Styling und JS-Code für die Interaktion zu schreiben; und dann dieses HTML direkt im Markdown-Text aufzurufen.

Moment mal? HTML + CSS + JS schreiben? Das erinnert mich an mein erstes Studienjahr.
...
-   Den Container für die schiebbare Galerie selbst
-   Die Bilder
...
Dank modernerer Frontend-Technologien ist die Umsetzung mit Flexbox heute sehr einfach. Die Kernidee ist, die Bilder in einer Reihe anzuordnen, aber nur den Platz für ein Bild freizugeben, während die anderen Bilder dahinter versteckt sind:
...
Der JS-Teil ist dafür verantwortlich, dass der Benutzer durch Interaktion steuern kann, welches Bild im sichtbaren Bereich angezeigt wird. Diese Interaktionen umfassen:

-   Klicken auf die linke/rechte Taste mit der Maus
-   Klicken auf die kleinen Punkte
...
Die grobe Idee des gesamten Codes ist, für jede schiebbare Galerie einen aktuellen Bild-Index zu speichern: `[0, 1, ..., imageNumbers - 1]`. Die Interaktion des Benutzers ändert den Index, woraus berechnet wird, um wie viele Pixel die Galerie verschoben werden muss, um das durch den Index angegebene Bild anzuzeigen.
...
Vom Interaktionslogik her müssen wir die Pixelposition des Fingers beim Start der Berührung und den Versatz der Fingerposition relativ zur Startposition bei jedem Render-Frame aufzeichnen. Dann wird bei jedem Frame-Rendering dieser Versatz auf die schiebbare Galerie angewendet, sodass das Bild dem Finger folgt. Nachdem der Finger eine Strecke bewegt wurde und den Bildschirm verlässt, müssen wir entscheiden, ob das Bild gewechselt werden soll. Zum Beispiel, wenn der Finger nur leicht den Bildschirm berührt und eine Rechtsverschiebung von 2 Pixeln verursacht, sollte das Bild nicht gewechselt werden; wenn der Finger jedoch über die Hälfte des Bildschirms nach rechts wischt, sollte das Bild gewechselt werden. Setzen wir einen Schwellenwert für den Versatz relativ zur gesamten Containerbreite auf 20%. Wenn die Wischstrecke 20% der Galeriebreite überschreitet, gehen wir davon aus, dass das Bild gewechselt werden soll.

API-seitig gibt es 3 relevante Ereignisse:

-   touchstart - Markiert den Beginn der Berührung, trägt die anfänglichen Fingerkoordinaten.
-   touchmove - Wird bei jeder Fingerbewegung ausgelöst, trägt die aktuellen Fingerkoordinaten.
-   touchend - Markiert das Verlassen des Bildschirms.

Rx eignet sich sehr gut für solche Anforderungen. Wir verpacken die Touch-Ereignisse des Benutzers in einen Datenstrom. Nach der Verarbeitung wird daraus ein Strom von horizontalen Finger-Versätzen, der verwendet wird, um die Position des Bildes in jedem Animationsframe zu aktualisieren. Schließlich wird nach Beendigung der Berührung basierend auf dem Verhältnis von Versatz zu Containerbreite entschieden, ob das Bild gewechselt werden soll.
