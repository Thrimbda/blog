...
"summary": "本文是一篇关于在 Kubernetes 单节点集群上手动搭建 Prometheus 监控系统的详细指南。文章首先介绍了搭建环境，包括
  K8s 版本 1.19.3 和 Prometheus 版本 2.22.0，并明确了不使用 Helm-Chart 或 Prometheus Operator
  等快捷部署方式，而是通过手动编写 YAML 配置文件来实现。内容分为概念验证和集群监控两部分：在概念验证阶段，作者演示了如何在裸机上运行
  Prometheus 和 Node Exporter，并修改配置文件以监控自身和节点指标；在集群监控阶段，详细讲解了 Prometheus 的核心配置项，如
  global、scrape_config、tls_config 等，并逐步部署了所需的 K8s 资源，包括
  Namespace、DaemonSet、ConfigMap、ServiceAccount、ClusterRole、Deployment 和
  Service。文章还提供了针对不同监控目标（如 Prometheus 自身、Node Exporter、Kubelet、Cadvisor 和
  ApiServer）的具体配置示例，并解释了 relabel_config 的作用。最后，作者总结了通过手动部署 Prometheus 来监控 K8s
  集群的关键步骤和注意事项，为初学者提供了实用的参考。"
"tags":
  - "可观测性"
  - "Prometheus"
  - "Kubernetes"
