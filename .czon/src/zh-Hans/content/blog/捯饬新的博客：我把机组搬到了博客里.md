...
"summary": "本文详细记录了作者因旧博客技术过时而重建新博客，并重点介绍了如何通过技术手段将机核网上的个人机组内容抓取、处理并集成到新博客中的完整过\
  程。文章从技术选型（Zola静态网站生成器）开始，逐步解析了从机核API获取数据、使用RxJS处理分页和数据结构转换、生成Markdown文件，再到利用Z\
  ola的Shortcodes功能实现滑动图片组前端交互的各个环节。作者不仅分享了具体代码实现，还探讨了前端开发带来的快速反馈循环所带来的快乐，最终表达了对\
  技术掌控力和创造过程纯粹乐趣的满足。"
"tags":
  - "博客"
  - "技术"
...
搭个人博客的风潮是在大三那年我们系流行来的。7 年前[这篇](./搭好博客后的一点话.md)令我脚趾抓地的文章介绍了最初我搭建这个博客的心路历程。
