...
"summary": "Este artículo detalla un proyecto Demo llamado 'Por Aiur' desarrollado por el autor para comprender el modelo RBAC. El proyecto simula un escenario simplificado del juego StarCraft, construyendo un servicio RESTful utilizando el framework Flask de Python e implementando el modelo RBAC dentro de este servicio. El artículo explica el diseño del Demo, incluyendo la definición de sujetos, roles, recursos y permisos, así como las relaciones de asignación sujeto-rol y rol-permiso. Además, explica cómo gestionar las relaciones RBAC mediante configuración de archivos sin usar una base de datos, y cómo combinar RBAC con la arquitectura RESTful. Finalmente, el artículo presenta la lógica de negocio y la implementación del código central del Demo, mostrando cómo controlar las operaciones sobre los recursos del juego a través de roles para lograr el objetivo del juego."
"tags":
  - "RBAC"
  - "RESTful"
  - "Flask"
...
No tengo la intención de hacer el Demo enormemente complejo. Mi objetivo es ilustrar el concepto, por lo que sigue siendo "pequeño pero completo".
...
Para que sea interesante y refleje plenamente el problema, simularé en mi Demo un escenario muy simplificado de una partida de StarCraft, donde cada unidad milagrosamente ha adquirido agencia subjetiva, desde la perspectiva de un jugador protoss.

Un buen Demo necesita un buen nombre, por lo que este Demo se llama: **¡Por Aiur!** (Aiur es el planeta natal de los protoss, y cada zealot dice esta frase apasionada al ser teletransportado al campo de batalla: "¡Lucho por Aiur!")

En **Por Aiur**, debes liderar a tu equipo para producir suficientes guerreros zealot, eliminar al destructor Amón y salvar el universo. Si no tienes suficientes zealots, fracasarás, las estrellas susurrarán y toda la existencia se desvanecerá.

Todo esto se refleja en un servicio RESTful escrito con el framework Flask de Python.

> En realidad, en un escenario real, cada sujeto excepto el jugador podría ser creado, por lo que todos podrían considerarse recursos. Para reflejar RBAC, les he añadido las siguientes restricciones:
>
> - Los pocos sujetos proporcionados por este Demo son únicos y existen permanentemente, como si fueran irrepetibles, eliminando así la posibilidad de que cualquier sujeto pueda ser creado recursivamente (por ejemplo: un probe puede construir un nexo, y un nexo puede producir probes).
> - En realidad, un portal necesita el apoyo energético de un pilar de cristal para funcionar, pero esto no se refleja aquí. Lo explico asumiendo que nuestro portal recibe energía de la *Spear of Adun*, la legendaria nave nodriza protoss en órbita síncrona, considerando así los pilones solo como recursos que proporcionan límite de población.
...
- Recolector de cristales (solo los probes pueden asumir este rol) - crystal_collector
...
> La relación muchos-a-muchos entre Sujeto-Rol y Rol-Permiso se representa usando la estructura de datos de tuplas múltiples de Python, y así es también en la implementación. Por lo tanto, esta aplicación no usa una base de datos.
...
En resumen, utilicé el framework Flask de Python para escribir un servicio con estilo RESTful. La aplicación completa no involucra una parte frontal, por lo que tampoco existen problemas de seguridad como eludir el frontend.

Una característica de este Demo es que no utiliza una base de datos. RBAC no obliga a usar una, y aunque usarla en RBAC es intuitivo y natural, en **Por Aiur** no usamos una base de datos. En su lugar, usamos archivos para representar las relaciones `sujeto-rol-permiso` de RBAC. Las bases de datos se desarrollaron sobre sistemas de archivos; aquí usamos archivos porque el sistema es lo suficientemente simple, para reducir aún más la complejidad del sistema con el fin de ilustrar el punto. La forma específica del archivo se puede ver en las descripciones de las relaciones SA y PA anteriores.
...
Y SE (Sesión) puede corresponderse muy bien con la sesión en una aplicación web, como un objeto temporal para un sujeto durante un inicio de sesión:

```python
# Sujeto utilizado para iniciar sesión en la API del sistema de gestión táctica de la Lanza de Adún. Aquí session es un objeto global de flask, cuyos detalles de implementación no se repetirán.
...
Y en las dos APIs utilizadas como ejemplo anteriormente, cada clase existe como un recurso en el sistema, y los métodos HTTP proporcionados son las operaciones sobre el recurso.

**Hasta aquí, todos los objetos de RBAC están presentes.**

### Lógica de Negocio

Dado que **Por Aiur** es una API de juego de estrategia en tiempo real jugable en línea, es necesario explicar su lógica de negocio:
...
La única condición para derrotar a Amón es tener un número suficiente de zealots, y este número es un entero aleatorio generado por el sistema entre 20 y 100. Simultáneamente, el sistema generará exactamente la cantidad de cristales sin recolectar necesarios para que puedas derrotar a Amón.

**¿Por qué exactamente suficientes?**
...
Toda esta lógica la proporciona un objeto cuyo ciclo de vida abarca toda la campaña. Para evitar problemas, he añadido un bloqueo de hilo para garantizar que cada operación sea atómica.

```python
# Como es el núcleo de todo el juego, lo llamo el Nexo - Nexus
import random
