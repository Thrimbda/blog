---
"title": "Spark 论文笔记"
"summary": "本文是一篇关于Spark论文的笔记，重点介绍了Spark框架的核心——弹性分布式数据集（RDD）。RDD是一种只读、可重建的分布式数据\
  结构，支持跨多个并行操作的数据复用。Spark通过Scala实现，利用其函数式和面向对象特性，提供了灵活的编程模型，包括RDD的创建方式（从文件系统、Sc\
  ala集合、已有RDD变换等）和持久化策略（cache和save）。文章概述了Spark的编程模型、并行操作（如reduce、collect）、实现细节（\
  基于Mesos、RDD接口设计）以及实验结果（性能优于Hadoop
  10倍）。最后，作者总结了Spark的早期成就和潜在价值，指出其在处理迭代和交互式工作负载时的优势。"
"tags":
  - "Spark"
  - "RDD"
  - "分布式系统"
