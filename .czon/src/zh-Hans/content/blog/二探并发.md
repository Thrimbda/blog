---
"title": "二探并发(Actor)"
"summary": "本文首先分析了在并发系统中使用共享消息队列时，当需要各worker各司其职异步工作时，会出现消息无法定向发送、错收消息导致效率低下和时\
  序问题。作者指出共享消息队列的FIFO特性及锁机制引入的效率问题，进而提出根治方法是使用Actor模型。Actor模型通过为每个worker设置独立的消息\
  队列（邮箱），实现消息的定向异步通信，避免了共享内存的复杂性。文章详细解释了Actor模型的概念，并给出了一个Python线程实现的示例代码，展示了如何通\
  过BaseWorker类实现Actor的基本功能，包括发送消息、接收消息和线程管理。最后，作者总结并发编程需要更抽象的思维方式，而Actor模型为入门提供\
  了指导。"
"tags":
  - "python"
  - "并发"
